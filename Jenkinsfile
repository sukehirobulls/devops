    ...
    stages {
        ...
        stage('Build Images') {
            steps {
                script {
                    // Nome das imagens
                    def imageNameWeb = 'mviniciusso/web'
                    def imageNameDB = 'mviniciusso/db'
                    def imageNameNginx = 'mviniciusso/nginx'

                    // Executa o build das imagens com os respectivos Dockerfiles
                    docker.withRegistry('https://registry.hub.docker.com', 'dockerhubmarcus') {
